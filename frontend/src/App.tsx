...
// Config: ใช้ค่าจาก Env หรือ Default
...
const App: React.FC = () => {
  ...
  const deleteUserLog = async (filenameKey: string, type: 'correct') => {
      try {
        ...
        // ⚠️ หมายเหตุ: Backend Python ต้องมี API นี้ด้วย (ถ้ายังไม่มีอาจ error 404)
  ...
  const downloadPersonalLog = async () => {
    ...
    try {
      ...
      a.download = filename;
      a.click();
    } catch (e) { alert('Error fetching personal log'); }
  };
    ...
    return (
      <div className="page-container center-content bg-slate-50 min-h-screen flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-8 text-center border border-slate-100">
          ...
          <p className="text-slate-400 mb-6 text-sm">Enter your ID to access workspace</p>
      ...
      <header className={`app-header ${window.scrollY > 10 ? "scrolled" : ""}`}>
        <div className="header-logo">
          ...
          <div className="flex flex-col">
            <span className="font-semibold text-slate-700">Audio Annotator</span>
            <span className="text-[10px] text-slate-400 font-mono tracking-wider">USER: {employeeId}</span>
          </div>
        </div>
      ...
      <main className="main-content animate-fade-in">
        ...
        {currentTab === "correct" && (
          <CorrectPage
            ...
            playingFile={playingFile}
            onInspectText={handleInspect}
            // availableFiles={availableFilenames} // ถ้า CorrectPage ต้องการใช้
          />
        )}
        {currentTab === "fail" && (
          <EditPage
